...
1. **Zero External Dependencies:**
   - Standard library ONLY (`struct`, `socket`, `math`, `collections`, `http.server`, `zipapp`, `unittest`, `hashlib`).
...
2. **Binary PCAP/PCAPNG Stream Readers & Live Socket Sniffer:**
   ...
   - Lightweight `PacketRef` (`__slots__`) storing packet metadata (`ts_us`, `file_offset`, `caplen`, `wirelen`, `flow_id`, `flags`, `tls_info`) without buffering payload bytes in RAM.
...
5. **Detection Engines (`netspector/modules/`):**
   ...
   - **Module 5 (`ja3_fingerprint.py`):** JA3 & JA4 TLS Client Hello fingerprinting & threat intelligence signatures (Cobalt Strike, AsyncRAT, Metasploit).
   - **Module 6 (`exfiltration.py`):** Data exfiltration byte volume anomalies & DNS tunneling payload inspector.
6. **Timeline Stitcher & PCAPNG Carver with Comment Injection:**
   ...
   - `netspector/carve.py`: Fast binary seek-and-copy PCAP/PCAPNG packet carver with option comment injection (`opt_comment`) for Wireshark details panel.
